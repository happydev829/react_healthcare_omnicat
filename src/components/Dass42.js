...
const Dass42 = () => {
  ...
  const validate = () => {
    ...
    for(let i = 0, valid = false; i < Object.keys(selections).length; i++) {
      valid = ( selections[ i ] === 0 || selections[ i ] === 1 ||
                    selections[ i ] === 2 || selections[ i ] === 3 )
      if (!valid) {
        log('not right values')
        return false
      }
    }
    return true
  ...
  return (
    <div className="dass">
      ...
      <form className="pure-form dass" onSubmit={handleSubmit}>
        {
          statements.map(
            (statement, index) => (
